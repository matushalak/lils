...
def plot_disentanglement_scores(overall_disent, disentanglement, completeness,
    ...
    sns.barplot(data=disentanglement.reset_index(), hue='Model',
                x='Latent', y='Disent', ax=ax1).set(ylim=(0, 1.03))
    sns.barplot(data=overall_disent.reset_index(), hue='Model',
                x='Model', y='Overall Disent',
                ax=ax2).set(ylim=(0, 1.03))
    sns.barplot(data=completeness.reset_index(), hue='Model',
...
def plot_disent_vs_nll(all_metrics, save_file):
    ...
    f = sns.relplot(kind='scatter', data=all_metrics,
                    x='Disentanglement', y='NLL', hue='Model',
    ...
    for name, g in all_metrics.groupby(['Model'], sort=False):
...
def disentanglement_vs_nll(raw_train, raw_test, end2end_models, gt_decoder,
    ...
    recons_metrics = pd.concat([all_train_nll, all_test_nll],
                               keys=score_names,
                               names=['Data', 'Model'])
    recons_metrics.name = 'NLL'

    disent = np.concatenate([overall_disent['Overall Disent'].values,
                             np.asarray([1.0])])
...
def run_analysis(end2end_models, gt_decoder, dataset,
    ...
    train = raw_train.get_supervised(pred_type='reg')
    test = raw_test.get_supervised(pred_type='reg')

    scores, results = run_daf(train, test, end2end_models,
                              method='lasso', assignment='optimal',
                              model_names=model_names)

    R_matrices = [r.hinton_matrix.T for r in results]
    overall_disent, disent, completeness = scores
    ...
    plot_disentanglement_scores(overall_disent, disent, completeness,
