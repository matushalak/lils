...
from ingredients.decoders import model, init_decoder as init_model
from ingredients.training import training, init_loader, init_metrics, \
                                 init_optimizer, reconstruction_loss, \
                                 ModelCheckpoint, init_loss
...
ex.add_config(no_cuda=False, save_folder='../data/temp/decoders')
...
# Dataset configs
dataset.add_config(dataset='dsprites', setting='reconstruction', shuffle=True)
...
def show():
    model = init_pred(latent_size=5, img_size=(3, 64, 64))
    print(model)
...
def main(_config):
    ...
    training_dataloader = init_loader(dataset)

    # Init model
    n_gen_factors = dataset.n_factors
    img_size = training_dataloader.dataset.img_size

    model = init_model(latent_size=n_gen_factors,
                       img_size=img_size).to(device=device)

    # Init metrics
    loss, metrics = init_loss(), init_metrics()
    optimizer = init_optimizer(params=filter(lambda p: p.requires_grad,
                                             model.parameters()))
